...
int main()
{
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //判断结点是叶结点
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //判断结点是父结点
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //判断此结点是子节点
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //判断此结点是父结点  
        {
            break;
        }
        if(huffman[i][3] < min2 && huffman[i][0] == -1 && i != ii[0])  //判断此结点是子节点
        {
            min2 = huffman[i][3];
            ii[1] = i;
        }
...
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
   ...
   ```c
   for(int k = 27; k < 53; k++) //遍历26个小写字母
           {
               if(str[i] == (char)(0x61 + k - 27))
               {
                   arr_big[k] += 1;
                   if(arr_big[k] == 1)
                   {
...
<img src="C:\Users\zl\AppData\Local\Temp\WeChat Files\bef7147dabc77fb0273eda211b76b38.png" alt="bef7147dabc77fb0273eda211b76b38" style="zoom:50%;" />

<img src="C:\Users\zl\AppData\Local\Temp\WeChat Files\264c6d7f62505c0eef058168cdfbfad.png" alt="264c6d7f62505c0eef058168cdfbfad" style="zoom:50%;" />
